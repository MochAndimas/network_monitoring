...
def _context(*, collection_status: str, system_status: str, metric_status: str) -> AlertEvaluationContext:
    device = Device(id=8, name="NAS Utama", device_type="nas")
    metrics = {
        (8, "nas_snmp_collection_status"): Metric(
            metric_value=collection_status, status="ok" if collection_status == "ok" else "warning"
        ),
        (8, "nas_system_status"): Metric(metric_value=system_status, status=metric_status),
    }
