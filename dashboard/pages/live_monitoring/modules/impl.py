"""Live monitoring data shaping, formatting, and rendering helpers."""
...
def _humanize_printer_text(value: str) -> str:
    """Convert printer status tokens into readable display text."""
...
def _y_axis_label(metric_name: str, unit: str | None) -> str:
    """Return y axis label for the live monitoring dashboard."""
...
def _format_duration(delta: pd.Timedelta | None) -> str:
    """Format duration for the live monitoring dashboard."""
...
def _format_metric_values(dataframe: pd.DataFrame) -> pd.Series:
    """Format metric values for the live monitoring dashboard."""
    unit_suffix = dataframe["unit"].map(lambda unit: f" {unit}" if _has_unit(unit) else "")
...
def _has_unit(unit: object) -> bool:
    """Return whether a metric unit should be rendered."""
