"""Metric persistence, latest snapshot, history, and daily summary queries."""

from collections.abc import Iterable

from shared.collection_utils import chunked
from shared.number_utils import safe_float
from sqlalchemy import Select, and_, case, desc, distinct, func, or_, select, tuple_
...
class MetricRepository:
    """Database access object for Metric records."""
...
def _is_metric_newer(metric: Metric, existing_checked_at, existing_metric_id: int | None) -> bool:
    """Return whether is metric newer applies in metric repository queries."""
...
def _next_uptime_streak_started_at(
    *,
    ...
    """Return next uptime streak started at for metric repository queries."""
...
def _rollup_statuses(statuses: list[str]) -> str:
    """Roll up rollup statuses in metric collection and history."""
