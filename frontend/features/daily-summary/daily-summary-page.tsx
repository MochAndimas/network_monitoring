...
import { initialPageSize } from "@/lib/pagination";
import { toWibDate } from "@/lib/wib-time";
...
export function DailySummaryPage() {
  const params = useSearchParams(); const [deviceId, setDeviceId] = useState(() => params.get("device") ?? ""); const [site, setSite] = useState(() => params.get("site") ?? ""); const [deviceType, setDeviceType] = useState(() => params.get("type") ?? ""); const [from, setFrom] = useState(() => params.get("from") ?? defaultRollupDate(7)); const [to, setTo] = useState(() => params.get("to") ?? defaultRollupDate()); const [offset, setOffset] = useState(() => initialOffset(params.get("offset"))); const [pageSize, setPageSize] = useState(() => initialPageSize(params.get("limit"), PAGE_SIZES, DEFAULT_PAGE_SIZE));
  useUrlQuerySync({ device: deviceId, site, type: deviceType, from, to, offset, limit: pageSize === DEFAULT_PAGE_SIZE ? undefined : pageSize });
  const query = useQuery({ queryKey: ["daily-summary", deviceId, site, deviceType, from, to, offset, pageSize], queryFn: () => apiFetch<Response>(withQuery("/metrics/daily-summary", { device_id: deviceId ? Number(deviceId) : undefined, site, device_type: deviceType, rollup_from: from, rollup_to: to, limit: pageSize, offset })) });
