...
export function AlertsPage() {
  ...
  const data = alerts.data; const items = data.items; const criticalHigh = items.filter((item) => ["critical", "high"].includes(item.severity.toLowerCase())).length; const affectedDevices = new Set(items.map((item) => item.device_name).filter(Boolean)).size; const range = pageRange(data.meta.total, data.meta.offset, items.length);
  ...
  return <main className="app-page"><PageHeader title="Alerts" description="Alert aktif, cakupan gangguan, dan prioritas respons operasional." />
    <MetaStrip items={[{ label: "Refresh otomatis", value: "Aktif (15 dtk)" }, { label: "Cakupan hasil", value: `${range.start}–${range.end} dari ${data.meta.total} alert` }, { label: "Filter", value: [severity, site, alertType, deviceId, search].filter(Boolean).join(" · ") || "Semua alert aktif" }, { label: "Terakhir dirender", value: formatWib(new Date().toISOString()) }]} />
