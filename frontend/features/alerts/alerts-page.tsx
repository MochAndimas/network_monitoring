...
const LIMIT = 50;
const severityRank: Record<string, number> = { critical: 0, high: 1, warning: 2, low: 3 };
...
export function AlertsPage() {
  ...
  const alerts = useQuery({ queryKey: ["alerts", severity, site, alertType, deviceId, search, offset], queryFn: () => apiFetch<AlertPage>(withQuery("/alerts/active/paged", { severity, site, alert_type: alertType, device_id: deviceId ? Number(deviceId) : undefined, search, limit: LIMIT, offset })), refetchInterval: 15_000 });
  ...
  const data = alerts.data; const items = [...data.items].sort((left, right) => sort === "severity" ? (severityRank[left.severity.toLowerCase()] ?? 99) - (severityRank[right.severity.toLowerCase()] ?? 99) || right.created_at.localeCompare(left.created_at) : right.created_at.localeCompare(left.created_at)); const criticalHigh = items.filter((item) => ["critical", "high"].includes(item.severity.toLowerCase())).length; const affectedDevices = new Set(items.map((item) => item.device_name).filter(Boolean)).size;
