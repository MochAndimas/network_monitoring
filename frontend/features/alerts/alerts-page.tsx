...
export function AlertsPage() {
  const [severity, setSeverity] = useState(""); const [site, setSite] = useState(""); const [search, setSearch] = useState(""); const [offset, setOffset] = useState(0);
  const alerts = useQuery({ queryKey: ["alerts", severity, site, search, offset], queryFn: () => apiFetch<AlertPage>(withQuery("/alerts/active/paged", { severity, site, search, limit: LIMIT, offset })), refetchInterval: 15_000 });
  ...
  const data = alerts.data; const criticalHigh = data.items.filter((item) => ["critical", "high"].includes(item.severity.toLowerCase())).length; const affectedDevices = new Set(data.items.map((item) => item.device_name).filter(Boolean)).size;
  const severityCounts = Object.entries(data.items.reduce<Record<string, number>>((result, item) => ({ ...result, [item.severity]: (result[item.severity] ?? 0) + 1 }), {}));
  const deviceCounts = Object.entries(data.items.reduce<Record<string, number>>((result, item) => ({ ...result, [item.device_name ?? "Unknown"]: (result[item.device_name ?? "Unknown"] ?? 0) + 1 }), {})).sort((left, right) => right[1] - left[1]).slice(0, 6);
  ...
  return <main className="app-page"><PageHeader title="Alerts" description="Alert aktif, cakupan gangguan, dan prioritas respons operasional." />
    <MetaStrip items={[{ label: "Refresh otomatis", value: "Aktif (15 dtk)" }, { label: "Filter", value: [severity, site, search].filter(Boolean).join(" · ") || "Semua alert aktif" }, { label: "Terakhir dirender", value: formatWib(new Date().toISOString()) }]} />
