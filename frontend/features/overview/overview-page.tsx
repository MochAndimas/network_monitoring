...
export function OverviewPage() {
  ...
  return <main className="app-page">
    ...
    <section className="two-column"><section><div className="section-header"><h2>Device Perlu Perhatian</h2><CsvExport filename="device-perlu-perhatian.csv" columns={["Device", "IP", "Tipe", "Site", "Status", "Dicek WIB"]} rows={problemRows.map((item) => [item.name, item.ip_address, item.device_type, item.site, item.latest_status, formatWib(item.latest_checked_at)])} /></div><DataTable columns={[{ key: "name", label: "Device", render: (item) => item.name }, { key: "ip", label: "IP Address", render: (item) => item.ip_address }, { key: "type", label: "Type", render: (item) => item.device_type }, { key: "site", label: "Site", render: (item) => item.site ?? "-" }, { key: "status", label: "Status", render: (item) => <StatusBadge value={item.latest_status} /> }, { key: "checked", label: "Pemeriksaan Terakhir (WIB)", render: (item) => formatWib(item.latest_checked_at) }]} rows={problemRows} /></section>
      <section><div className="section-header"><h2>Distribusi Tingkat Alert</h2></div><PlotlyChart ariaLabel="Distribusi tingkat alert aktif" data={[{ type: "bar", orientation: "h", x: Object.values(data.alert_severity_summary), y: Object.keys(data.alert_severity_summary), marker: { color: "#f7c948" }, hovertemplate: "%{y}: %{x}<extra></extra>" }]} layout={{ xaxis: { title: { text: "Jumlah alert" } }, yaxis: { automargin: true, autorange: "reversed" } }} /><DataTable columns={[{ key: "severity", label: "Tingkat", render: ([severity]) => <StatusBadge value={severity} /> }, { key: "count", label: "Jumlah", render: ([, count]) => count }]} rows={Object.entries(data.alert_severity_summary)} /></section></section>
    ...
    <section><div className="section-header"><h2>Snapshot Metric Terbaru</h2><CsvExport filename="snapshot-metric-terbaru.csv" columns={["Dicek WIB", "Device", "Metrik", "Nilai", "Status"]} rows={data.latest_snapshot.items.map((item) => [formatWib(item.checked_at), item.device_name, item.metric_name, `${item.metric_value}${item.unit ? ` ${item.unit}` : ""}`, item.status])} /></div><DataTable columns={[{ key: "checked", label: "Dicek (WIB)", render: (item) => formatWib(item.checked_at) }, { key: "device", label: "Device", render: (item) => item.device_name }, { key: "metric", label: "Metrik", render: (item) => item.metric_name }, { key: "value", label: "Nilai", render: (item) => `${item.metric_value}${item.unit ? ` ${item.unit}` : ""}` }, { key: "status", label: "Status", render: (item) => <StatusBadge value={item.status} /> }]} rows={data.latest_snapshot.items} /></section>
  </main>;
}
