...
export function OverviewPage() {
  ...
  return <main className="app-page">
    ...
    <section className="two-column"><section><div className="section-header"><h2>Device Perlu Perhatian</h2><CsvExport filename="device-perlu-perhatian.csv" columns={["Device", "IP", "Tipe", "Site", "Status", "Dicek WIB"]} rows={problemRows.map((item) => [item.name, item.ip_address, item.device_type, item.site, item.latest_status, formatWib(item.latest_checked_at)])} /></div><DataTable columns={[{ key: "name", label: "Device", render: (item) => item.name }, { key: "ip", label: "IP Address", render: (item) => item.ip_address }, { key: "type", label: "Type", render: (item) => item.device_type }, { key: "site", label: "Site", render: (item) => item.site ?? "-" }, { key: "status", label: "Status", render: (item) => <StatusBadge value={item.latest_status} /> }, { key: "checked", label: "Pemeriksaan Terakhir (WIB)", render: (item) => formatWib(item.latest_checked_at) }]} rows={problemRows} /></section>
      <section><h2>Distribusi Tingkat Alert</h2><DataTable columns={[{ key: "severity", label: "Tingkat", render: ([severity]) => <StatusBadge value={severity} /> }, { key: "count", label: "Jumlah", render: ([, count]) => count }]} rows={Object.entries(data.alert_severity_summary)} /></section></section>
