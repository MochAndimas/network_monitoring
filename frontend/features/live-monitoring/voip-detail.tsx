...
export function VoipDetail({ samples, selectedMetric, windowHours }: { samples: MetricSample[]; selectedMetric: string; windowHours: number }) {
  const groups = numericSamplesByMetric(samples, windowHours).filter(([name]) => !selectedMetric || name === selectedMetric);
  return <section>
    <h2>Tren Semua VoIP</h2>
    <p>Setiap warna pada grafik mewakili satu device VoIP; kartu memakai agregasi nilai terbaru antar-device.</p>
    {groups.length ? groups.map(([metricName, values]) => {
      ...
      return <section key={metricName}>
        <h3>{metricLabel(metricName)} - Semua VoIP</h3>
